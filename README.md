...
INF2005 ACW1 · Done by Team P3-6. First draft on Junyong's branch, based on Gabriel's `54a5789` frontend and Kim's PNG steganalysis implementation.
...
4. **Verify — Bob:** upload the received object, use the agreed LSB depth/location,
   and explicitly supply Alice's trusted public key. Bob's key pair is not required.
   An in-app shortcut helps local testing, as the assignment demonstration must also
   show an actual Alice-to-Bob transfer and verification of Bob's downloaded copy.
5. **Compare:** original/stego preview for comprison, hidden-content preview for the last
...
Carrier traversal is row-major channel order for images and interleaved sample order for WAV. 
The current image carrier includes alpha for RGBA images; steganalysis excludes alpha and reports that limitation.
...
Every returned verdict contains the same evidence fields and six different cases:
extraction, record binding, signature, content integrity, cover integrity and
decryption (not applicable). Status include Passed, Failed, Not run and Not
applicable with unavailable values shown explicitly.

The different verdict classifications are as follows:
...
The application does not claim **Wrong Start Location** merely because extraction fails.
...
- Chi-square p-values are not probabilities that secret data exists. RS uses mask asymmetry, not an estimated payload length. Thresholds are provisional; both false positives and missed payloads are possible. Higher-depth analysis is exploratory. An Authentic file can also show hidden-data indicators.
